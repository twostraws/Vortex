...
extension VortexSystem {
    ...
    func update(date: Date, drawSize: CGSize) {
        ...
        if settings.isEmitting
            && lastUpdate - lastIdleTime > settings.emissionDuration
        {
            settings.isEmitting = false
            lastIdleTime = lastUpdate
        } else if settings.isEmitting == false
            && lastUpdate - lastIdleTime > settings.idleDuration
        {
    ...
    func createParticle(force: Bool = false) {
        guard settings.isEmitting, emissionCount < settings.emissionLimit ?? Int.max || force == true else { return }
    ...
    func spawn(from particle: Particle, event: VortexSettings.SpawnOccasion) {
        ...
        let matchedSettings = settings.secondarySettings.filter {
